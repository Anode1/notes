...
Source control should contain the same application code for every environment. The artifact should contain built application bits and deployment logic, but not environment-specific runtime data. That runtime data has two different categories. The first category is true secrets: production passwords, test passwords, SMTP credentials, database passwords, API tokens, private keys, OAuth secrets, and similar values. The second category is operational configuration: database connection strings, SMTP hosts, internal service URLs, bucket names, service identifiers, feature flags, and environment names. The first category must be protected as secret. The second category may not always be secret, but it still defines the environment in which the artifact runs. Both belong to the runtime environment, not to source control and not to the artifact.
...
The mechanism can be simple or sophisticated. A Linux service may read a protected file such as \texttt{/etc/<app>/system.properties}. A Windows service may read a protected configuration file from a service-owned directory. A container platform may mount configuration or inject environment variables. Larger platforms may use a parameter store, vault, key vault, managed identity, secret manager, or internal configuration service. Public endpoints can be documented openly when they are truly public. Internal service URLs and other environment coordinates are different: they are not necessarily secrets, but they are still operational configuration and should be supplied by the environment. The architectural rule is the same:

\begin{quote}
Code is in the repository. Built application bits are in the artifact. Secrets and environment-specific operational configuration are supplied at runtime.
\end{quote}
